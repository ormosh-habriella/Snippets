...
def test_view_snippets_button_on_homepage(browser, live_server):
    ...
    expected_url = f"{live_server.url}{reverse('snippets-page')}"
...
def test_navigation_buttons_for_unauthorized_user(browser, live_server):
    ...
    assert len(my_snippets_buttons) == 0, \
        "Кнопка 'Мои сниппеты' не должна быть доступна для неавторизованного пользователя"
...
def test_snippets_my_page_access_and_content(browser, live_server):
    ...
    user1_num_snippets = 12
    user2_num_snippets = 14
    user1_snippets = SnippetFactory.create_batch(user1_num_snippets, user=user1, public=True)
    user2_snippets = SnippetFactory.create_batch(user2_num_snippets, user=user2, public=True)

    # Тест 1: Проверяем, что неавторизованный пользователь не может получить доступ
    try:
        browser.get(f"{live_server.url}{reverse('my-snippets')}")
    ...
    assert current_url == f"{live_server.url}{reverse('my-snippets')}", \
    ...
    # Проверяем количество сниппетов (должно быть n для user1)
    # total_snippets = browser.find_element(By.XPATH, '//i[contains(text()="Всего сниппетов")]')
    total_snippets = browser.find_element(By.ID, 'total_snippets')
